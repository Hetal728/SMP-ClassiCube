...
class CoreServerProtocol(Protocol):
    ...
    def dataReceived(self, data):
        ...
        while self.buffer:
            ...
            if type == TYPE_INITIAL:
               ...
               # correct_pass = hashlib.md5(self.factory.salt + self.username).hexdigest()[-32:]

              #  if mppass.strip("0") == correct_pass.strip("0"):
                    #self.identified = True
               # if mppass == correct_pass:
                #    self.username += "+"
                 #   self.identified = True
                #if self.identified == False:
                 #   self.log("Kicked '%s'; authentication failed")
    ...
    def HandleExtEntry( message ):
        extensions.append(enc.get_string(message, 0, 64).strip())

    def HandleCustomBlockSupportLevel():
        customBlockSupportLevel = "1"

    def SendExtEntry(self, name, version):
        buffer = Array.CreateInstance(Byte, 68)
        self.StringFormat(name, 64).CopyTo(buffer, 0)
        self.HTNO(version).CopyTo(buffer, 64)
        self.SendRaw(17, buffer)

    def StringFormat(str, size):
    ...
    def HTNO(self, x):
        y = BitConverter.GetBytes(x)
        Array.Reverse(self._y)
        return self._y

    def SendRaw(self, id):
        self.SendRaw(id, Array.CreateInstance(Byte, 0))

    def SendRaw(self, id, send):
        buffer = Array.CreateInstance(Byte, send.Length + 1)
        buffer[0] = id
        Buffer.BlockCopy(send, 0, buffer, 1, send.Length)
        try:
            socket.BeginSend(buffer, 0, buffer.Length, SocketFlags.None, None)
        except:
            self.connectionLost(self, "Error in SendRaw")
